...
def test(args):
    ...
    * wbs_dictionary_path: A path to a file containing a list of words that the wbs should constrain to
    ...
    for images, labels in dataset:
        ...
        input_lengths = tf.constant(np.full((iter_batch_size,), configs[MAX_SEQ_SIZE]))
        ...
        highest_loss = max(highest_loss, max(loss))
        loop.update(1)
        loop.set_description(f'Highest loss {highest_loss:.6f}')
    loop.close()

    file_path = configs[CSV_PATH]
    with open(file_path, 'r') as f:
        data = f.readlines()
    
    file_path = file_path[:-4] + '_losses.tsv'
    with open(file_path, 'w') as f:
        for d, l in zip(data, losses):
            d = d.strip()
            f.write(f'{d}\t{l}\n')
